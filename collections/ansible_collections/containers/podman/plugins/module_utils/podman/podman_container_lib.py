...
ARGUMENTS_SPEC_CONTAINER = dict(
    ...
    state=dict(
        type="str",
        default="started",
        choices=["absent", "present", "stopped", "started", "created"],
    ),
    image=dict(type="str"),
...
class PodmanContainerDiff:
    ...
    def diffparam_device(self):
        before = [
            ":".join([i["pathonhost"], i["pathincontainer"]])
    ...
    def diffparam_memory_swap(self):
        ...
        if (
            self.module_params["memory_swap"] is None
    ...
    def diffparam_network(self):
        ...
        if net_mode_after and not before:
            ...
            return self._diff_update_and_compare(
                "network", net_mode_before, net_mode_after
            )
        before, after = sorted(list(set(before))), sorted(list(set(after)))
        return self._diff_update_and_compare("network", before, after)
    ...
    def diffparam_ulimit(self):
        ...
        if after:
            ...
            before = {
                u["name"].replace("rlimit_", ""): "%s:%s" % (u["soft"], u["hard"])
                for u in ulimits
            }
