...
class PodmanImageManager(object):
        ...
        if self.push:
            ...
            self.results["actions"].append(
                push_format_string.format(
                    image_name=self.image_name, dest=self.push_args["dest"]
                )
            )
            self.results["changed"] = True
            if not self.module.check_mode:
    ...
    def push_image(self):
        ...
        if not dest:
            if "/" not in self.name:
                self.module.fail_json(
                    msg="'push_args['dest']' is required when pushing images that do not have the remote registry in the image name"
                )
        ...
        # remove it and warn since it's not needed.
            ...
            self.module.warn(
                "Image name and tag are automatically added to push_args['dest']. Destination changed to {dest}".format(
                    dest=dest
                )
            )
        ...
        if transport:
            if not dest:
                self.module.fail_json(
                    "'push_args['transport'] requires 'push_args['dest'] but it was not provided."
                )
            if transport == "docker":
        ...
        if rc != 0:
            self.module.fail_json(
                msg="Failed to push image {image_name}: {err}".format(
                    image_name=self.image_name, err=err
                )
            )
